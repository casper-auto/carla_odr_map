...
MapLaneVector OdrMap::GetMapLanes(const Road& road,
    ...
    for (const auto& laneRoadMarks : roadMarkStartSEndSInfos) {
        ...
        for (const auto& markIdSPairMap : laneRoadMarks.second) {
            odr_map_msgs::LaneInformation laneInformation;
 ...
 */
const odr_map_msgs::MapObject& OdrMap::GetAllMapObjects() const {
...
*/
std::vector<odr_map_msgs::MapBoundary> OdrMap::GetMapBoundaries(const Road& road, double step) {
    // type==curb
    RoadMarkStartSEndSInfos roadMarks = GetRoadMarkStartEndInfos(road, RoadMarkType::Curb);

    std::vector<odr_map_msgs::MapBoundary> mapBoundaries;
    for (const auto& laneRoadMarks : roadMarks) {
        ...
        for (const auto& markIdSPairMap : laneRoadMarks.second) {
            odr_map_msgs::MapBoundary mapBoundary;
...
odr_map_msgs::MapObject OdrMap::GetMapObjectWithinTargetRoads(const Sequence& road_ids) const {
    odr_map_msgs::MapObject mapObject;
 ...
 * @return
 */
odr_map_msgs::MapObject OdrMap::GetMapObject(float x, float y, float z, double distance) {
...
MapBasePathVector OdrMap::GetMapBasePaths(Road& road, double step) {
    ...
    for (const auto& laneStartSEndS : laneStartSEndSInfos) {
        odr_map_msgs::MapBasepath mapBasepath;
...
void OdrMap::ReverseDirection(odr_map_msgs::MapLane& mapLane) {
 ...
 * @return
 */
void OdrMap::AddPathConnection(std::vector<odr_map_msgs::MapBasepath> path_connection_list) {
